...
module.exports = {
	...
	run: async (client, message, args, cmduser, text, prefix) => {
		...
		if (!channel) {return message.reply({embeds: [new MessageEmbed().setTitle('<:NO:1169479454918180937> You need to join a voice channel').setColor(es.wrongcolor).setFooter(client.getFooter(es))]});}
		...
		if (botchannel) {return message.reply({embeds: [new MessageEmbed().setTitle(`<:NO:1169479454918180937> I am already connected in: \`${botchannel.name}\``).setFooter(client.getFooter(es))]});}
