    ...
    try {
      ...
      if(!channel)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:1016981253997735987> **You forgot to ping a CHANNEL**`)
        ]});
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
      ...
      await channel.join().then(()=>{
        message.reply({embeds :[new MessageEmbed()
          .setColor(es.color)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:yes:1016992526504300544> **I joined \`${channel.name}\`**`)
        ]});
      }).catch(()=>{
        message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:yes:1016992526504300544> **I couldn't join \`${channel.name}\`**`)
        ]});
      })
