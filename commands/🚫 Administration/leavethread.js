    ...
    try {
      ...
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:1016981253997735987> **\`${channel.name}\` Channel is not a Thread**`)
        ]});
      await channel.leave().then(()=>{
        if(channel.id != message.channel.id){
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:yes:1016992526504300544> **I left \`${channel.name}\`**`)
          ]});
        }
      }).catch(()=>{
        message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:yes:1016992526504300544> **I couldn't leave \`${channel.name}\`**`)
        ]});
      })
